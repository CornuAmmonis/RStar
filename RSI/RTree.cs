...
namespace RSI
{
    public class RTree : SpatialIndex
    {

        // parameters of the tree
        private const int DEFAULT_MAX_NODE_ENTRIES = 50;
        private const int DEFAULT_MIN_NODE_ENTRIES = 20;
        internal int maxNodeEntries;
        internal int minNodeEntries;
        internal bool useInternalConsistencyChecking;
        ...
        public RTree()
        {
            maxNodeEntries = RSI.Properties.Settings.Default.MaxNodeEntries;
            minNodeEntries = RSI.Properties.Settings.Default.MinNodeEntries;
        ...
        ///  
        ///  MaxNodeEntries: This specifies the maximum number of entries
        ///  in a node. The default value is 10, which is used if the property is
        ///  not specified, or is less than 2.
        ///  MinNodeEntries: This specifies the minimum number of entries
        ...
        public bool Delete(Rectangle r, int id)
        {
            ...
            while (foundIndex == -1 && parents.Count > 0)
            {
                ...
                if (!n.IsLeaf())
                {
                    log.Debug("deletion: searching node " + n.nodeId + ", from index " + startIndex);
                    bool contains = false;
            ...
            if (size == 0)
            {
                root.mbrMinX = float.MaxValue;
                root.mbrMinY = float.MaxValue;
                root.mbrMaxX = -float.MaxValue;
                root.mbrMaxY = -float.MaxValue;
            }
        ...
        public void Nearest(Point p, Invokable<int> v, float furthestDistance)
        {
            ...
            ForEach(nearestIds, v);

            // Need an O(1) Reset().
            nearestIds.Clear();
        }

        private void ForEach(List<int> l, Invokable<int> v)
        {
            foreach (int i in l)
            {
                v.Invoke(i);
                ...
                else
                {
                    ...
                    for (int i = 0; i < n.entryCount; i++)
                    {
                        ...
                        if (entryDistanceSq <= furthestDistanceSq)
                        {
                            ...
                            if (savedValues.Count > 0 && savedPriority == distanceQueue.GetPriority())
                            {
        ...
        public void NearestNUnsorted(Point p, Invokable<int> v, int count, float furthestDistance)
        {
            ...
            // same priority.
            PriorityQueue distanceQueue = new PriorityQueue(log, PriorityQueue.SORT_ORDER_DESCENDING);
            CreateNearestNDistanceQueue(p, count, distanceQueue, furthestDistance);

            while (distanceQueue.Size() > 0)
            {
                v.Invoke(distanceQueue.GetValue());
                distanceQueue.Pop();
            }
        }

        public void NearestN(Point p, Invokable<int> v, int count, float furthestDistance)
        {
            PriorityQueue distanceQueue = new PriorityQueue(log, PriorityQueue.SORT_ORDER_DESCENDING);
            CreateNearestNDistanceQueue(p, count, distanceQueue, furthestDistance);
        ...
        /// </summary>
        private float Nearest(Point p, Node n, float furthestDistanceSq, List<int> nearestIds)
        {
        ...
        /// </summary>
        private bool Intersects(Rectangle r, Invokable<int> v, Node n)
        {
            for (int i = 0; i < n.entryCount; i++)
            {
        ...
        private Node ChooseNode(float minX, float minY, float maxX, float maxY, int level)
        {
            ...
            // TODO: Make sure Clear() has the right behavior here. Used TIntStack.clear()
            parents.Clear();
            parentsEntry.Clear();
        ...
        private bool CheckConsistency(int nodeId, int expectedLevel, Rectangle expectedMBR)
        {
